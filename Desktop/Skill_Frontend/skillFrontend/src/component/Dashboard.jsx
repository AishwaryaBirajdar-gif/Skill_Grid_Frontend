    ...
    <div className="min-h-screen bg-[#F8FAFC] pb-20 font-sans">
      ...
      <nav className="bg-white border-b border-slate-200 px-8 py-4 sticky top-0 z-50 shadow-sm">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center justify-center gap-8 text-sm font-bold text-slate-500 flex-1">
            <button onClick={() => navigate("/")} className="flex items-center gap-2 hover:text-indigo-600 transition-colors">
              <Home size={18}/> Home
            </button>
          ...
          <div className="flex justify-end w-1/4">
            <div className="flex items-center gap-3 bg-slate-100 px-5 py-2.5 rounded-full cursor-pointer hover:bg-indigo-100 transition-all border border-transparent hover:border-indigo-200" onClick={() => navigate("/profile")}>
      ...
      <div className="max-w-7xl mx-auto px-6 pt-12 space-y-12">
        ...
        >
          <div className="relative z-10">
            ...
            <p className="text-indigo-100 text-lg font-medium opacity-90 max-w-2xl leading-relaxed">
              ...
              Manage them in your <strong className="cursor-pointer underline" onClick={() => navigate("/profile")}>Profile Section</strong>.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          <StatCard icon={<Clock />} label="Pending Requests" value={pendingRequests.length} color="text-rose-600" bg="bg-rose-50" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          <BigBox 
            ...
            desc="Curated matches based on your desired skills. Start your next learning journey." 
          ...
          <BigBox 
            ...
            desc="Track the status of the skills you've requested from others." 
            icon={<Send size={40} className="text-blue-500" />} 
          />
          <BigBox 
            title="Active Rooms" 
            desc="Ongoing exchanges. Jump back into your chats and collaborative sessions." 
            icon={<MessageSquare size={40} className="text-emerald-500" />} 
          />
          <BigBox 
            title="Reputation & Karma" 
            desc="Current Balance: 120 Points. Your status reflects your helpfulness in the community." 
            icon={<Award size={40} className="text-amber-500" />} 
          />
          <BigBox 
            title="Trending Skills" 
            desc="Explore the most popular skills being traded this week on SkillGrid." 
...
const BigBox = ({ title, desc, icon, badge, onClick }) => (
  <div 
    onClick={onClick}
    className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm hover:border-indigo-400 hover:shadow-2xl hover:shadow-indigo-100/50 transition-all duration-500 cursor-pointer group relative overflow-hidden"
  >
    {badge && (
      <span className="absolute top-8 right-8 bg-rose-500 text-white text-[11px] font-black px-4 py-1.5 rounded-full uppercase tracking-widest">
        {badge}
      </span>
    )}
    <div className="flex flex-col gap-8">
      <div className="w-20 h-20 bg-slate-50 rounded-[1.8rem] flex items-center justify-center group-hover:scale-110 group-hover:bg-indigo-50 transition-all duration-500">
        {icon}
      </div>
      <div>
        ...
        <div className="mt-8 text-indigo-600 text-xs font-black uppercase tracking-[0.2em] flex items-center gap-3 group-hover:gap-5 transition-all">
